...
/***************************************************************************//**
 * @brief
 *   Driver to open and set an LETIMER peripheral in PWM mode. Scheduler is also
...
void letimer_pwm_open(LETIMER_TypeDef *letimer, APP_LETIMER_PWM_TypeDef *app_letimer_struct){
  ...
  letimer->ROUTEPEN = (app_letimer_struct->out_pin_0_en * LETIMER_ROUTEPEN_OUT0PEN) | (LETIMER_ROUTEPEN_OUT1PEN * app_letimer_struct->out_pin_1_en);
	 ...
	 */
	  ...
	  letimer->REP0 = PWM_rep;
	  letimer->REP1 = PWM_rep;
	  ...
	  // enable registers
	  letimer->IEN |= (app_letimer_struct->comp0_irq_enable * LETIMER_IEN_COMP0);
	  letimer->IEN |= (app_letimer_struct->comp1_irq_enable * LETIMER_IEN_COMP1);
	  letimer->IEN |= (app_letimer_struct->uf_irq_enable * LETIMER_IEN_UF);
	  // LETIMER_IF |= (LETIMER_IF_COMP0 | LETIMER_IF_COMP1 | LETIMER_IF_UF)
...
/***************************************************************************//**
 * @brief
 * set a uint32_t int_flag to the value of the letimer0 interrupt flag, if statements
 * process this value and act accordingly
 *
 * @note
 * comp0,comp1, and uf are the values detected and actioned on
 *
 *
 ******************************************************************************/
...
/***************************************************************************//**
 * @brief
 * Manage the lowest energy sleep mode and still maintain letimer operation
 *
 * @note
 * A call should also be made to establish the energy mode that the microcontroller \
 * cannot enter while the LETIMER is active only if the LETIMER was not enabled at the
 * start. Additionally that call should only be made once to the block mode sleep
 * routine.
 *
 *@param[in]
 * letimer struct encompasses status values of letimer, enable bool determines y | n
 ******************************************************************************/
